...
app.layout = html.Div([
    html.Div([html.H1("Enhanced ECG/EEG Real-time Monitor with AI Analysis",
    ...
    html.Div([
        ...
        html.Label("Select Patients:", style={"marginTop": "8px"}),
        dcc.Dropdown(id="patients-dropdown", multi=True, style={"width": "400px"}),

        html.Div([
            html.Label("Select Channels to Display:", style={"marginTop": "8px"}),
            dcc.Dropdown(id="channels-dropdown", multi=True, style={"width": "400px"},
                         placeholder="Select channels (leave empty for auto-selection)"),

            html.Label("Display Mode:", style={"marginTop": "8px"}),
            dcc.RadioItems(
                id="overlay-mode",
                options=[
                    {"label": "Overlay on same graph", "value": "overlay"},
    ...
    html.Div([
        html.H3("AI-Based ECG Analysis", style={'color': '#00ff41'}),
        html.P("Choose analysis method for current ECG signal data",
        ...
        html.Div([
            html.Button("Run 1D AI Analysis", id="ai-analyze-btn", n_clicks=0,
                        style={"marginTop": "8px", "marginRight": "10px", "backgroundColor": "#007acc",
                               "color": "white", "border": "none", "padding": "10px 20px",
                               "borderRadius": "6px", "fontSize": "14px", "fontWeight": "bold",
                               "cursor": "pointer"}),
            html.Button("Run 2D AI Analysis", id="ai-analyze-2d-btn", n_clicks=0,
                        style={"marginTop": "8px", "backgroundColor": "#00aa00", "color": "white",
...
def load_data(nc, dataset_type, data_dir):
    ...
    if patients:
        ...
        channel_info = f"{dataset_type}: {len(available_channels)} channel(s) available. Select channels to display or leave empty for auto-selection."
...
def run_ai_analysis(n_clicks_1d, n_clicks_2d, selected_patients, selected_channels,
    ...
    try:
        ...
        if is_2d_analysis:
            if signal_type != "ECG":
                return html.Div([
                    html.H4("2D AI Analysis:", style={"color": "#00ff41"}),
            ...
            if "ecg" not in patient or patient["ecg"] is None:
                return html.Div([
                    html.H4(f"2D AI Analysis - {patient_name}:", style={"color": "#00ff41"}),
                    html.P("No data available for analysis", style={"color": "#ff6347"})
                ])
            ...
            try:
                ...
                if not available_channels:
                    return html.Div([
                        html.H4("2D AI Analysis:", style={"color": "#00ff41"}),
                        html.P("No channels available", style={"color": "#ff6347"})
                    ])
                ...
                if img_bytes is None:
                    return html.Div([
                        ...
                        html.Pre("pip install kaleido",
                                 style={"color": "#ccc", "fontSize": "12px", "fontFamily": "monospace",
                                        "backgroundColor": "#1a1a1a", "padding": "10px", "borderRadius": "5px"})
                    ])
                ...
                result_elements.append(
                    html.H4(f"2D AI Analysis - {patient_name}:",
                            style={"color": "#00ff41", "marginBottom": "10px"})
                )
                ...
                if result.get("success"):
                    if not result.get("requires_setup"):
                        ...
                        result_elements.append(
                            html.Div([
                                html.P("Analysis Complete!",
                                       style={"color": "#00ff41", "fontWeight": "bold", "fontSize": "16px",
                                              "marginBottom": "10px"}),
                                html.Div([
                                    html.Span("Top Prediction: ",
                                              style={"color": "#ccc", "fontSize": "14px"}),
                                    html.Span(f"{top_pred}",
                                              style={"color": "#00bfff", "fontSize": "16px", "fontWeight": "bold"})
                                ], style={"marginBottom": "8px"}),
                                html.Div([
                                    html.Span("Confidence: ",
                                              style={"color": "#ccc", "fontSize": "14px"}),
                                    html.Span(f"{top_conf * 100:.1f}%",
                                              style={"color": "#00ff41", "fontSize": "16px", "fontWeight": "bold"})
                                ])
                            ], style={"backgroundColor": "#003300", "padding": "15px",
                        ...
                        result_elements.append(
                            html.P("All Predictions:",
                                   style={"color": "#00bfff", "fontWeight": "bold", "marginTop": "15px",
                                          "marginBottom": "10px"})
                        )
                                ...
                                html.Div([
                                    ...
                                    html.Div(
                                        style={
                                            ...
                                            "marginTop": "5px",
                                            "borderRadius": "3px",
                                            "boxShadow": f"0 0 10px {bar_color}"
                                        }
                                    )
                        ...
                        result_elements.append(
                            html.Div([
                                html.P(f"Image Size: {result.get('image_size', 'N/A')} | "
                                       f"Model: Teachable Machine",
                                       style={"color": "#888", "fontSize": "10px", "margin": "0"})
                            ], style={"marginTop": "15px", "padding": "8px",
                                      "backgroundColor": "#1a1a1a", "borderRadius": "5px"})
                        )
                        ...
                        result_elements.append(
                            html.Div([
                                html.P("Setup Required:",
                                       style={"color": "#ffd700", "fontWeight": "bold", "fontSize": "16px",
                                              "marginBottom": "10px"}),
                                html.P(
                                ...
                                html.Pre("pip install tensorflow tensorflowjs",
                                         style={"backgroundColor": "#1a1a1a", "padding": "12px",
                                                "borderRadius": "5px", "color": "#00ff41",
                                                "fontSize": "13px", "fontFamily": "monospace",
                                                "border": "1px solid #00ff41"}),
                                html.P("Then restart the application to enable real-time predictions.",
                                       style={"color": "#ccc", "fontSize": "12px", "marginTop": "10px"})
                            ], style={"backgroundColor": "#332200", "padding": "15px",
                        ...
                        if class_labels:
                            result_elements.append(
                                html.P("Detected Model Classes:",
                                       style={"color": "#00bfff", "fontWeight": "bold", "marginTop": "15px",
                                              "marginBottom": "10px"})
                            )
                            for i, label in enumerate(class_labels):
                                result_elements.append(
                                    html.Div(f"{i + 1}. {label}",
                    ...
                    result_elements.append(
                        ...
                        ], style={"marginTop": "10px", "padding": "15px", "backgroundColor": "#330000",
                                  "borderRadius": "5px", "border": "2px solid #ff6347"})
                    )

                # Timestamp
                result_elements.append(
                    html.P(f"Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                           style={"color": "#666", "fontSize": "10px", "marginTop": "15px",
                                  "textAlign": "right"})
                )
        ...
        # ========== 1D SIGNAL-BASED ANALYSIS (Original) ==========
                ...
                try:
                    ...
                    if channel_warning:
                        patient_elements.append(
                            html.P(channel_warning,
                                   style={"color": "#ffd700", "fontSize": "11px",
                                          "backgroundColor": "#332200", "padding": "5px",
                                          "borderRadius": "3px", "marginBottom": "8px"})
                        )

                    # Analysis info
                    signal_length = analysis_result.get("signal_length", len(signal_data))
                    inference_time = analysis_result.get("inference_time_s", 0)
                    model_type = analysis_result.get("model_type", "unknown")
                    leads_used = analysis_result.get("leads_used", 1)

                    patient_elements.append(
                        html.P(f"Model: {model_type.upper()} | Channels: {leads_used} | "
                               f"Samples: {signal_length} | Time: {analysis_duration:.2f}s",
                               style={"color": "#ccc", "fontSize": "11px", "fontStyle": "italic"})
                    )
                    ...
                    for i, pred in enumerate(predictions[:5]):  # Show top 5
                        ...
                        if confidence >= 0.8:
                            ...
                            confidence_color = "#ff8c00"
                            icon = "△"
                            border_color = "#ff8c00"
                        else:
                            confidence_color = "#ff6347"
                            icon = "·"
                            border_color = "#ff6347"
                    ...
                    # Channel info
                    patient_elements.append(
                        html.P(f"Available channels: {', '.join(available_channels[:5])}"
            ...
            if all_results:
                ...
                all_results.append(
                    html.Div([
                        html.P([
                            html.Span("AI Model Info: ",
                                      style={"color": "#ccc", "fontSize": "11px", "fontWeight": "bold"}),
                            html.Br(),
                            html.Span(f"Signal Type: ", style={"color": "#888", "fontSize": "11px"}),
                            html.Span(f"{model_info.get('signal_type', 'N/A')}",
                                      style={"color": "#00bfff", "fontSize": "11px"}),
                            html.Span(" | ", style={"color": "#555"}),
                            html.Span(f"Model: ", style={"color": "#888", "fontSize": "11px"}),
                            html.Span(f"{model_info.get('model_type', 'N/A')}",
                                      style={"color": "#00bfff", "fontSize": "11px"}),
                            html.Br(),
                            html.Span(f"Classes: ", style={"color": "#888", "fontSize": "11px"}),
                            html.Span(f"{model_info.get('num_classes', 0)}",
                                      style={"color": "#00bfff", "fontSize": "11px"}),
                            html.Span(" | ", style={"color": "#555"}),
                            html.Span(f"Predictions Made: ", style={"color": "#888", "fontSize": "11px"}),
                            html.Span(f"{model_info.get('total_predictions', 0)}",
                                      style={"color": "#00bfff", "fontSize": "11px"}),
                        ], style={"margin": "5px 0"})
                    ])
                )
            ...
            if not all_results:
                return html.Div([
                    html.P("No analysis results available.", style={"color": "#ff6347"}),
...
def make_viz_figure(viz_type, patients, selected_idxs, show_all_channels, state,
    ...
    try:
        ...
        # ------------------------
            ...
            # Use data up to current playback position for real-time update
            ...
            # Get time values for angle calculation
            ...
            if overlay:
                # Overlay all channels on same polar plot
                fig = go.Figure()
                ...
                fig.update_layout(
                    template="plotly_dark",
                    title=f"Polar View: {p.get('name', 'Patient')} - {len(plot_data)} samples",
                    polar=dict(
                        radialaxis=dict(
                            showticklabels=True,
                            ticks='',
                            range=[0, max_r * 1.1] if max_r > 0 else [0, 1],
                            title=dict(text=f"Amplitude ({unit})")
                        ),
                        angularaxis=dict(
                            showticklabels=True,
                            ticks='',
                ...
                # Separate polar subplots for each channel
                n_channels = len(channels_to_display)

                # Calculate grid layout
                cols = min(2, n_channels)  # Max 2 columns for polar plots
                ...
                for idx, ch in enumerate(channels_to_display):
                    ...
                    # Update polar axes for this subplot
        ...
        # ------------------------
            ...
            if len(channels_to_display) % 2 != 0:
                dropped = channels_to_display[-1]
                channels_to_display = channels_to_display[:-1]
                print(f"[crossrec] Dropped '{dropped}' to form pairs (need even number of channels)")

            pairs = [(channels_to_display[i], channels_to_display[i + 1])
                     for i in range(0, len(channels_to_display), 2)]

            if not pairs:
                return create_empty_figure("No valid channel pairs")

            # Use data up to current playback position
            current_data = p["ecg"].iloc[:pos]
            ...
            # Build recurrence matrices comparing actual amplitude values
            rec_matrices = []
            pair_titles = []
            pair_info = []
            vmax = 0
            ...
            for ch_a, ch_b in pairs:
                # Get the actual signal amplitude values up to current position
                s1 = current_data[ch_a].values.astype(float)
                s2 = current_data[ch_b].values.astype(float)

                mlen = min(len(s1), len(s2))
                if mlen == 0:
                    rec_matrices.append(None)
                    pair_titles.append(f"{ch_a} vs {ch_b} (empty)")
                ...
                # Create amplitude bins for comparison
                n_bins = 80  # Number of amplitude bins (reduced for better real-time performance)

                # Define amplitude ranges for both channels
                s1_min, s1_max = s1.min(), s1.max()
                s2_min, s2_max = s2.min(), s2.max()

                # Ensure we have a valid range
                if s1_max - s1_min < 1e-6:
                    s1_min, s1_max = s1_min - 1, s1_max + 1
                if s2_max - s2_min < 1e-6:
                    s2_min, s2_max = s2_min - 1, s2_max + 1

                # Create 2D histogram: how often does channel A have amplitude X when channel B has amplitude Y?
                hist, xedges, yedges = np.histogram2d(
                    s1, s2,
                    bins=n_bins,
                    range=[[s1_min, s1_max], [s2_min, s2_max]]
                )
                ...
                pair_titles.append(f"{ch_a} vs {ch_b} ({mlen} samples)")
                pair_info.append({
                    'xedges': xedges,
                    'yedges': yedges,
            ...
            for idx, (rec, info) in enumerate(zip(rec_matrices, pair_info)):
                ...
                if rec is None or rec.size == 0 or info is None:
                    continue

                # Use actual amplitude values for axes
                hm = go.Heatmap(
                    z=rec.T,  # Transpose so y-axis is channel B
                    x=info['xedges'][:-1],  # Channel A amplitude values
                    y=info['yedges'][:-1],  # Channel B amplitude values
                    colorscale=[[0, 'black'], [0.3, '#003300'], [0.6, '#00aa00'], [1, '#00ff41']],
                    zmin=0,
                    zmax=max(vmax, 0.01),  # Ensure non-zero max
                    showscale=(idx == len(rec_matrices) - 1),
                    colorbar=dict(
                        title="Co-occurrence<br>Frequency",
                        len=0.6
                    ) if idx == len(rec_matrices) - 1 else None,
                    hovertemplate=(
                        f"{info['ch_a']} amplitude: %{{x:.2f}} {unit}<br>"
                        f"{info['ch_b']} amplitude: %{{y:.2f}} {unit}<br>"
                        "Frequency: %{z:.3f}<br>"
                        "<extra></extra>"
                    )
                )
                ...
                # Update axes with amplitude labels
                fig.update_xaxes(
                    title_text=f"{info['ch_a']} Amplitude ({unit})",
                    row=r, col=c
                )
                fig.update_yaxes(
                    title_text=f"{info['ch_b']} Amplitude ({unit})",
                    row=r, col=c
                )

            fig.update_layout(
                template="plotly_dark",
                title=f"Cross-Recurrence (Real-time): {p.get('name', 'Patient')} - Position: {pos}/{len(p['ecg'])}",
                height=max(450, 300 * rows)
            )

            # Add explanation
            fig.add_annotation(
                text="Bright regions show amplitude values that frequently occur together (updates in real-time)",
...
def combined_update(n_intervals, n_play, n_pause, n_reset, state,
    ...
    try:
        patients = GLOBAL_DATA.get("patients", [])
        if not patients:
            return create_empty_figure("No patients loaded"), create_empty_figure(), {
        ...
        import traceback
        traceback.print_exc()
        return create_empty_figure("Error occurred"), create_empty_figure(), {
            "playing": False, "pos": [], "write_idx": []
        }
